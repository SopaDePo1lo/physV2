...
class DeformableObject(Object):
    ...
    def draw(self, screen, colour):
        ...
        for i in range(NUMP-1):
            pygame.draw.aaline(screen, colour, self.myPoints[i].posTupel(),self.myPoints[i+1].posTupel())
        pygame.draw.aaline(screen, colour, self.myPoints[-1].posTupel(),self.myPoints[0].posTupel())

    def draw_springs(self, screen, colour):
        for spring in self.mySprings:
            p1,p2 = self.myPoints[spring.i],self.myPoints[spring.j]
            pygame.draw.aaline(screen, colour, (p1.x, p1.y), (p2.x, p2.y))
